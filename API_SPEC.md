...
## 4. 스토어 컨텍스트 파일 관리 `/store-context`

판매자가 등록한 txt 파일을 AI 서버에 저장하고 챗봇이 이를 참조합니다.

### **POST** `/store-context/upload` — 파일 업로드

`multipart/form-data` 형식으로 전송합니다.

| 필드 | 타입 | 설명 |
|------|------|------|
| store_id | string | 판매 페이지 ID (BE의 페이지 ID 그대로 사용) |
| file | file | txt 파일 (최대 5000자) |

```json
// Response
{ "store_id": "store_001", "char_count": 142, "ok": true }
```

- 같은 `store_id`로 재업로드 시 덮어쓰기
- 서버 재시작 후에도 파일 유지 (디스크 저장)

### **DELETE** `/store-context/{store_id}` — 파일 삭제

```json
// Response
{ "store_id": "store_001", "deleted": true }
```

---

## 5. AI 챗봇 `/chatbot` `/chatbot/stream`
...
`store_id` 또는 `store_context` 중 하나를 반드시 포함합니다. `store_id` 우선입니다.
...
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| message | string | Y | 고객 메시지 |
| history | array | N | 이전 대화 (최대 20개) |
| store_id | string | Y* | 업로드된 txt 파일 ID |
| store_context | string | Y* | 직접 전달하는 텍스트 (하위 호환용) |

> *`store_id`와 `store_context` 중 하나 필수
...
#### 욕설 감지 시 흐름 (경고)
```
[즉시]       data: {"token": "", "is_warning": true, "can_answer": true, "final": "욕설이 포함된 메시지는..."}
             → thinking 없이 즉시 반환, LLM 호출 없음
             → BE가 is_warning: true 수신 시 warning_count +1
             → warning_count >= 3이면 BE에서 대화 차단
```
...
| 욕설 경고 | `is_warning: true`, `final: "..."` | 경고 메시지 표시, BE에서 카운트 관리 |
| 오류 | `error: "..."` | 오류 메시지 표시 |
...
const response = await fetch('http://localhost:8000/chatbot/stream', {
  ...
  body: JSON.stringify({ message, history, store_id })  // store_id 사용
});
...
while (true) {
  ...
  for (const line of decoder.decode(value).split('\n')) {
    ...
    if (data.is_warning)   { setMessage(data.final); /* BE에서 경고 카운트 처리 */ }
    if (data.final && !data.is_warning) { setMessage(data.final); setShowHandoffButton(true); }
  }
}
```
