...
11. 고객 문의 작성, 상담, 주문/스토어, 운영자 workspace의 mock 의존을 줄이고 DB/API 기반 흐름으로 전환했다.
...
추가로 DB/API 연결이 완료된 영역:

- 고객 문의 작성과 내 문의 조회
- 고객 상담 세션과 상담 메시지 저장/조회
- 고객 주문/스토어 목록 조회
- 운영자 workspace의 주문, 상담, 문의, 문의 답변 조회
- 운영자 내부 메모
- 운영자 응답 프리셋
- 운영자 챗봇 지식 파일 목록
- 스토어별 FAQ
- 운영자 txt 지식 파일 원문 저장/미리보기/다운로드

현재 비어 있거나 아직 최소 구현만 된 주요 영역:
...
목업 스토어마다 별도 운영자 계정을 만들고 `store.owner_user_id`로 스토어와 연결했다.

| store | operator login_id | password |
|---|---|---|
| 패션스토어 루미 | `operator01` | `1234` |
| 맛있는 빵집 | `operator_bakery` | `1234` |
| 테크존 전자 | `operator_techzone` | `1234` |
| 그린라이프 마트 | `operator_greenlife` | `1234` |
| 북카페 서랍 | `operator_bookcafe` | `1234` |

운영자 화면은 로그인한 운영자가 소유한 스토어 기준으로 주문, 상담, 문의를 조회한다.

기존 데모 row도 함께 보정했다.

- `store.owner_user_id`를 스토어별 운영자 계정으로 연결
- `support_session.operator_user_id`를 해당 스토어 운영자로 연결
- 운영자 상담 메시지와 문의 답변 작성자를 해당 스토어 운영자로 연결

### 8.4 FAQ DB 답변 전환

FAQ는 `faq_article`에 저장된 스토어별 데이터를 사용한다.

- `GET /api/stores/{store_id}/faqs`로 활성 FAQ를 조회한다.
- 고객 챗봇 화면의 FAQ 버튼은 AI 서버를 호출하지 않는다.
- FAQ 클릭 시 DB의 answer/content를 즉시 채팅 메시지로 표시한다.
- 자유 입력 질문은 기존처럼 `/api/ai/chatbot/stream`을 사용한다.

### 8.5 운영자 txt 지식 파일 업로드

운영자 설정의 `챗봇 적용 정보 관리`는 파일명을 직접 입력하지 않고 실제 `.txt` 파일을 업로드한다.

- 드래그 앤 드롭 또는 `업로드` 버튼으로 `.txt` 파일을 선택한다.
- DB에는 업로드한 원본 파일명이 `chatbot_knowledge_file.file_name`으로 저장된다.
- txt 원문은 `chatbot_knowledge_file.file_content`에 저장된다.
- 목록의 미리보기 버튼은 저장된 txt 원문을 화면에 표시한다.
- 목록의 다운로드 버튼은 저장된 txt 원문을 원본 파일명으로 내려받는다.
...
### 14.2 문의/상담 DB 저장 후 남은 점

고객 문의 작성, 상담 세션/메시지, 운영자 문의 답변, 내부 메모는 DB/API 기반으로 전환됐다.

남은 작업:

- 문의/상담 상태 전환 UI의 세부 polish
- 첨부 업로드 저장 흐름
- 통계/KPI 화면의 DB 집계 전환

### 14.3 프론트 목업 데이터와 DB 데이터 혼재

현재 주요 고객/운영자 화면은 DB 응답을 사용하지만, 일부 보조 UI에는 아직 mock fallback이 남아 있다.

주의:

- 고객 주문/스토어/상담/문의, 운영자 workspace/settings는 DB 응답 기반이다.
- 일부 화면 표시용 기본값과 실패 시 fallback은 남아 있다.
- 장기적으로는 화면 표시용 mock fallback도 DB seed 기준으로 줄이면 좋다.
