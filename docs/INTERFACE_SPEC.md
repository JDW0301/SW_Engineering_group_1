...
- **인증 헤더**: `Authorization: Bearer {accessToken}` (BE 전용, AI는 인증 없음)
...
**FE 연동 예시 (React)**
```javascript
const res = await fetch('http://203.234.62.47:8000/chatbot/stream', {
  method: 'POST',
...
| `store_context` | — | string | FE→AI | 스토어 정보 (챗봇 컨텍스트) |
...
| `store_context` 포맷 — 어디서 생성해 AI로 전달하는지 | BE에서 조립해 FE로 주거나 FE에서 직접 조립 | 조동욱↔윤건호↔조규상 |
...
[ ] FE에서 AI /detect 호출 → is_profanity 반환 확인
[ ] FE에서 AI /chatbot/stream SSE 수신 확인
[ ] HANDOFF 시나리오: can_answer=false → 상담사 연결 버튼 노출 확인
[ ] 욕설 시나리오: is_profanity=true → cleaned 텍스트 표시 확인
[ ] /classify 호출 → category 반환 확인
```
