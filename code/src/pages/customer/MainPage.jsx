...
const MainPage = ({ setPage, openStore, inquiries, openInquiry, user }) => (
  <div className="space-y-6">
    ...
    <section>
      ...
      {inquiries.filter(i => i.status !== "RESOLVED" && i.type === "상담").length === 0 ? (
        <p className="text-sm text-gray-400 py-4 text-center">진행 중인 상담이 없습니다</p>
      ) : inquiries.filter(i => i.status !== "RESOLVED" && i.type === "상담").sort((a, b) => b.lastMessageAt.localeCompare(a.lastMessageAt)).slice(0, 5).map(inq => (
