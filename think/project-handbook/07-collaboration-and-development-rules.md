...
- 이 프로젝트의 기본 브랜치는 `main`, `AI`, `BE`, `FE`, `test` 다섯 개만 사용한다.
- `main` 브랜치에 직접 push하지 않는다.
- `AI`, `BE`, `FE`, `test` 브랜치도 직접 push로 정리하지 않고 Pull Request를 통해 변경 이력을 남긴다.
- 기능별 임시 브랜치(`feat/...`, `fix/...`)는 기본 전략으로 사용하지 않는다.

### 5.2 브랜치 역할 규칙

각 브랜치의 역할은 다음과 같다.

- `FE`: 프론트엔드 화면, 상태, UI 상호작용 관련 작업
- `BE`: 백엔드 API, 인증, DB 접근, 서버 로직 관련 작업
- `AI`: 챗봇, 분류, 요약, 안전 보조 등 AI 관련 작업
- `test`: 영역별 브랜치에서 완료된 변경을 통합하고 검증하는 브랜치
- `main`: 최종적으로 검토와 검증을 마친 안정 브랜치

규칙:

- 프론트엔드 변경은 기본적으로 `FE` 브랜치에서 관리한다.
- 백엔드 변경은 기본적으로 `BE` 브랜치에서 관리한다.
- AI 변경은 기본적으로 `AI` 브랜치에서 관리한다.
- 두 개 이상 영역에 걸치는 변경은 관련 영역 브랜치에 각각 반영한 뒤 `test`에서 통합 검증한다.
- `main`에는 `test`에서 검증이 끝난 내용만 병합한다.
...
`test` 브랜치는 선택 사항이 아니라 기본 통합 브랜치로 사용한다.
영역 브랜치에서 끝난 작업은 `test`에서 먼저 합쳐 보고 검증한 뒤 `main`으로 올린다.
...
4. 작업은 `main`, `AI`, `BE`, `FE`, `test`의 고정 브랜치 전략을 따른다.
...
- [ ] 변경이 올바른 고정 브랜치(`main`, `AI`, `BE`, `FE`, `test`) 흐름을 따랐다.
...
```text
문서 확인 → 해당 영역 브랜치 작업 → 기능 개발 → 문서/네이밍 반영 → test 검증 → PR 검토 → main 병합
```
